...
class TestOrganizationList(unittest.TestCase):
    excel = HandleExcel('OrganizationList')   # 创建HandleExcel对象
    ...
    @data(*cases)
    def test_OrganizationList(self, case):
        # src_data = case.data
        # re.sub(r'{not_existed_tel}', '', src_data)
        # 1. 参数化
        new_data = Parameterize.to_param(case.data)
        new_data = eval(new_data)
        new_data = new_data.get("userId")

        # 2. 拼接完整的url
        new_url = do_yaml.read('api', 'prefix') + case.url + new_data
        ...
        try:
            ...
            self.assertEqual(expected_result, actual_value.get('success'), msg=msg)
